...
class TestTeleportPlatformActivate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.galactica.get_interior().add_player((0, 0), Player('Mike'))
        self.teleport, self.player = \
            self.galactica.get_interior().get_objects((0, 0))

    def test_no_spaceships_in_teleport_radius(self):
        self.teleport.set_player(self.player)
    ...
    def test_teleport_player_to_adjacent_spaceship(self):
        ...
        self.assertIn(self.player,
                      enterprise.get_interior().get_objects((0, 0)))
